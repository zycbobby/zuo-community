function pmm_solve_with_similarity_weighted()
...
pmm_solve_betweenness(betweenness,mid);
end

function pmm_solve_betweenness(betneenness,mid)
% figure out the solution iteratively

global adj_content adj_content_full

adj_content=adj_content_full.*(adj_content_full>=mid).*(EBC_53<betweenness);

for k=4:10
    for l=4:2:16
        [q1,q2,com_list]=pmm_solve_ret(k,l);
        fname=strcat('community_',num2str(sim_criterion),'_',num2str(k),'_',num2str(l),'.list');
        fid=fopen(fname,'w');
        fprintf(fid, '{"modularity1":%f, "modularity2":%f,"product":%f,"k":%d,"l":%d, "sim_criterion":%f}\n',q1,q2,q1*q2,k,l,sim_criterion);
